...
class StateActionFeatureVectorWithTile():
    def __init__(self,
        ...
        for i in range(len(state_low)):
            dimension_size = math.ceil((state_high[i] - state_low[i]) / tile_width[i]) + 1
            self.dimension_num.append(dimension_size)
            self.feature_len *= dimension_size

        # Calculate single feature length with and without action
    ...
    def __call__(self, s, done, a) -> np.array:
            ...
            for k in range(self.num_tilings):
                ...
                # Set the corresponding feature element to 1
